...
import { FolderOpen, Code2, FileCode2, BarChart3, AlertCircle, Loader2, Info, FileArchive } from 'lucide-react';
...
export default function App() {
  ...
  const handleZipSelection = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        for (const [relativePath, zipEntry] of Object.entries(zip.files)) {
          ...
          if (ext && LANGUAGE_MAP[ext]) {
            try {
              setCurrentFile(`[${file.name}] ${relativePath}`);
              ...
              console.error(`Failed to read ${relativePath} in ${file.name}`, err);
            }
          }
        }
      }
      ...
      setError(err.message || 'An error occurred while scanning the ZIP files.');
    } finally {
      setIsScanning(false);
      setCurrentFile('');
      if (zipInputRef.current) zipInputRef.current.value = ''; // reset
    }
  };
    ...
    <div className="min-h-screen bg-[#09090b] text-zinc-100 font-sans selection:bg-blue-500/30">
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-6 py-12 md:py-20">
        <div className="flex flex-col items-center text-center mb-16">
          ...
          >
            Analyze your local projects securely in the browser. Select a folder or upload ZIP files to recursively scan and count lines of code across all supported languages.
          </motion.p>
          ...
          >
            <button
              ...
              disabled={isScanning}
              className={cn(
                "group relative inline-flex items-center justify-center gap-3 px-8 py-4 rounded-full font-medium text-white transition-all duration-300 w-full sm:w-auto",
            ...
            >
              <FileArchive className="w-5 h-5 transition-transform group-hover:-translate-y-0.5" />
